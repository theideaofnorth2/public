...
export function* storieTransition(fromStorie, toStorie) {
...
const isIntroduction = state =>
  state.app.view === "waiting" || state.app.view === "home";
...
function* onExplorationClick(arg) {
  ...
  const currentStorie =
    stateBeforeSelection.stories.data[lastPastIndex(stateBeforeSelection)];
  ...
  if (isIntroduction(stateAfterSelection)) {
    ...
    if (arg.mode === "tour") {
      yield put({ type: "STORIE_SELECTION", index: 1 });
      yield storieTransition(currentStorie, secondStorie);
    }
    return true;
  }
  if (arg.mode === "interactive") {
    yield put({ type: "STORIE_SELECTION", index: 0 });
