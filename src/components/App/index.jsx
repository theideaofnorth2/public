...
export class MyComponent extends Component {
  componentWillReceiveProps(nextProps) {
    if (this.props.controls.hovered && !nextProps.controls.hovered)
      this.setMouseStopTimer();
    else if (!this.props.controls.hovered && nextProps.controls.hovered)
      this.clearMouseStopTimer();
  }
  onMouseMove = () => {
    if (!this.props.controls.hovered) this.setMouseStopTimer();
    if (!this.props.controls.visible)
      this.props.dispatch({ type: "CONTROLS_SHOW" });
  };
  ...
  render() {
    if (this.props.app.isCapture) {
      ...
      return (
        <div className={css.app} onMouseMove={this.onMouseMove}>
          <Layer4 />
        </div>
      );
    }
