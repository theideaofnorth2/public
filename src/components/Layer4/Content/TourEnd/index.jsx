...
export class MyComponent extends Component {
	...
	initLinks = () => {
		const approachLink = this.contentRef.querySelector('[data-tion2-link=approach]');
		...
		const interactiveLink = this.contentRef.querySelector('[data-tion2-link=interactive]');
		if (approachLink) approachLink.addEventListener('click', this.approachHandler);
		...
		if (interactiveLink) interactiveLink.addEventListener('click', this.interactiveHandler);
	}
	approachHandler = () => {
	...
	tourHandler = () => {
		this.props.dispatch({ type: 'EXPLORATION_CLICK', mode: 'tour', confirm: false });
		this.props.dispatch({ type: 'EXIT_CONTENT_CLICK' });
	}
	interactiveHandler = () => {
		this.props.dispatch({ type: 'EXPLORATION_CLICK', mode: 'interactive', confirm: false });
		this.props.dispatch({ type: 'EXIT_CONTENT_CLICK' });
	}
	render() {
		...
		return (
			<div className={thisClass}>
				<div
					...
					dangerouslySetInnerHTML={{ __html: this.props.pages.data.tourEnd[language] }}
				/>
			</div>
		);
	}
