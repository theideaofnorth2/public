...
export default function reducer(state = defaultState, action = null) {
  switch (action.type) {
    case "CONFIG_READY": {
      const guides = action.data.guides.map(guide =>
        Object.assign({
          view: guide.view,
    ...
    case "STORIE_SELECTION": {
      const pastData = state.data.slice(0, action.index + 1).map(storie =>
        Object.assign({}, storie, {
          future: false,
        }),
      );
      const futureData = state.data
        .slice(action.index + 1, state.data.length)
        .map(storie =>
          Object.assign({}, storie, {
            future: true,
          }),
